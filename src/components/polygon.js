...
class Polygon {
     ...
     * @param {Coordinates} point is an object consisting x,y coordinates
     ...
     *
     */
    isPointInside(point,polygon_vertices){
        let is_odd = false;
        for (let i = 0, j = polygon_vertices.length - 1; i < polygon_vertices.length; j = i++) { //looping through all edges
            ...
            if (this.yCoordinateBetweenVerticesYRange(point.y,vertexI.y,vertexJ.y) &&
                this.xOnLeftHalfPlane(point,vertexI,vertexJ) )
                is_odd = !is_odd;
        }
        return is_odd;
    }
     ...
     * @param {Coordinates} point is a y coordinate of the point
     ...
     * @returns {Boolean} true when the y coordinate is between the y's, otherwise returns false
     */
    xOnLeftHalfPlane(point,vertexI,vertexJ){
